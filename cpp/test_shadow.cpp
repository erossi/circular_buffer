...
#include <iostream>
#include <cstdio>
#include "circular_buffer_shadow.h"
...
void help()
{
	...
	cout << " r : Reset the shadow index." << endl;
	cout << " y : Commit the shadow index." << endl;
	cout << " q : Quit." << endl;
...
void printit(const CBufferS<uint8_t, uint8_t>& cbuffer)
{
	printf("\n");
...
int main() {
	CBufferS<uint8_t, uint8_t> cbuffer {BUF_SIZE};
	uint8_t *message;
	uint8_t len;
	bool FLloop {true};
	uint8_t rxc;
	...
	cout << endl << "Test circular buffer (shadow index)." << endl;
